 // Ce bouton permet d'envoyer une alerte Bonjour suivie de la valeur saisie par l'utilisateur (son prénom)  ------------------------------------------------------------------------
 function first() {
	 // Cette ligne permet de récupérer la valeur saisi par l'utilisateur avec l'id "prenom"
            let prenom = document.getElementById("prenom").value; 
			// Affiche une alerte indiquant le message "bonjour" suivi du prénom saisi par l'utilisateur 
            alert("Bonjour " + prenom); 
        }


// Ce bouton permet d'Afficher/ Masquer le bouton Mon CV  ------------------------------------------------------------------------				
function MonCV() {
	// Cette ligne récupère le contenu du CV avec l'id "aboutme"
    const apropos = document.getElementById("aboutme"); 
	// Cette ligne permet de récupérer le bouton avec l'id "monCV"
    const bouton = document.getElementById("monCV");     

	// Cette ligne récupère le style d'affichage réel de l'élément (tel qu'il est calculé par le navigateur)
    const currentDisplay = window.getComputedStyle(apropos).display;

	// Le bloc if permet d'indiquer que si l'élément est caché on l'affiche et on change le texte du bouton par "Masquer mon CV"
    if (currentDisplay === "none") {
        apropos.style.display = "block";
        bouton.innerHTML = "Masquer mon CV";
		// Sinon si l'élément est visible on le cache et on change le texte du bouton par "Afficher mon CV"
    } else {
        apropos.style.display = "none";
        bouton.innerHTML = "Afficher mon CV";
    }
}
...
// Ce bouton permet d'Afficher/ Masquer le bouton Aide 	(nous avons repris le modèle proposé par Mr BRIGLIA en complétant les x manquants en s'aidant de cette ressource : https://www.w3schools.com/css/css_display_visibility.asp)  ------------------------------------------------------------------------
 function Aide() {
...
// Cela permet de charger le texte (nous avons repris le modèle proposé par Mr BRIGLIA sans modification)  ------------------------------------------------------------------------
...
//Bouton segmentation (nous avons repris le modèle proposé par Mr BRIGLIA en apportant certaines modifications tel que le nom des ID) ------------------------------------------------------------------------
function segmentation() {
	// La boucle if permet d'indiquer que si aucun fichier n'est chargé alors cela affiche un message qu'il faut charger un fichier
...
function dictionnaire() {
    ...
    // Cela permet de vérifier qu'un texte à bien été chargé 
    if (texte === "") {
        document.getElementById("page-analysis").innerHTML = "Il faut d'abord charger le fichier sous format txt.";
        return; // l'instruction return permet d'arrêter immédiatement l’exécution de la fonction : https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Statements/return
    }

    // Ici, on effectue la tokenisation du texte
    const mots = texte
        .toLowerCase()
        .split(/\s+/)
        .map(mot => mot.trim())
    ...
    // Ici, on calcule les fréquence de chaque token
    ...
    // Ici on affiche le tableau créé précédemment au niveau de l'id page-analysis 
...
// Le bouton nombrePhrases permet de compter le nombre de phrase dans un fichier txt  ------------------------------------------------------------------------
...
// Cette ligne permet de couper le texte en phrases à chaque ponctuation qui est suivie d'un espace, supprime les espaces inutiles d'un mot et ignore les phrases vides
    ...
    let tableau = '<table><tr><th>Mot</th><th>Longueur</th></tr>';
